...
plot(1:9, z1^2/sum(z1^2)) #Scree plot- variance by each features
plot(1:9, cumsum(z1^2/sum(z1^2))) #Total/aggregate variance by no. of features

cumsum(z1^2/sum(z1^2)) #1st 5 will account for ~85% of the variance; keep 5
...
pca1$center #update comment

pca1$scale

head(pca1$x) #take this to regression algorithms

##clean up lines 643 to 647
g<-as.data.frame(pca1$x)

g%>%ggplot(aes(x = PC1, y = PC2)) +
  geom_point(aes(col=X1$play_type), size=1)+
  ggtitle("PCA of Play Type")
```
...
dx<-data.frame(as.data.frame(pca2$x[,1:8]))
...
auc.train <- performance(predR, measure = "auc")
...
auc.train1 <- performance(predRQ, measure = "auc")
auc.train1 <- auc.train1@y.values

#Plot ROC- QDA
plot(roc.perf1,main="LDA")
abline(a=0, b= 1) #Ref line indicating poor performance
text(x = .40, y = .6,paste("AUC = ", round(auc.train[[1]],3), sep = ""))
